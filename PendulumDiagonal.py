...
def simulate_bifurcation(alphas,
    ...
    def derivs(t1, w1, t2, w2):
        δ = t1 - t2
        denom = 2.0 + 1.0 - math.cos(2.0*δ)  # 2*m1+m2 - m2*cos(2δ), con m1=m2=1
        dω1 = (
...
h_min_counts = min_counts.copy_to_host()
h_max_vals   = max_values.copy_to_host()
h_min_vals   = min_values.copy_to_host()

# reconstruimos listas para graficar
alpha_max = []
theta2_max = []
alpha_min = []
theta2_min = []
...
for i in range(n_alpha):
    cmax = min(h_max_counts[i], max_per_alpha)
    for j in range(cmax):
        alpha_max.append(alphas[i])
        theta2_max.append(h_max_vals[i, j])
    cmin = min(h_min_counts[i], max_per_alpha)
    for j in range(cmin):
        alpha_min.append(alphas[i])
        theta2_min.append(h_min_vals[i, j])

# convertimos a grados y mostramos
alpha_max_deg = np.degrees(np.array(alpha_max))
theta2_max_deg = np.degrees(np.array(theta2_max))
alpha_min_deg = np.degrees(np.array(alpha_min))
theta2_min_deg = np.degrees(np.array(theta2_min))

plt.figure(figsize=(8,6))
plt.scatter(alpha_max_deg, theta2_max_deg, s=0.1, edgecolor='none', label='máximos')
plt.scatter(alpha_min_deg, theta2_min_deg, s=0.1, edgecolor='none', label='mínimos')
plt.xlabel("Ángulo inicial α (grados)")
plt.ylabel("θ₂ (grados)")
plt.title("Máximos y mínimos del péndulo doble (10 000 iteraciones)")

...
\begin{titlepage}
  ...
  % --- Código QR con enlace principal ---
  \qrcode[height=2.6cm]{https://github.com/ColaCaos/ColaCAOS/tree/master/quarto/caos-memoria/memoria.pdf}\\[0.2cm]
...
\begin{itemize}
\item
  En el capítulo \nameref{efecto-mariposa} se muestra la sensibilidad extrema a las condiciones
...
\item
  En la el capítulo \nameref{mapa-de-fases} se demuestra vía simulación que el péndulo doble es caótico.
...
\item
  En \nameref{caos-en-las-predicciones-meteorologicas} se recopilan las predicciones
...
\item
  En
  \nameref{evaluacion-de-las-predicciones-climuxe1ticas}, las predicciones climáticas
...
\item
  En
  \nameref{simulaciuxf3n-y-funcionamiento-cauxf3tico} se analiza el comportamiento caótico de un
  sistema inventando por mí. El atractor resultante es muy parecido al
  de Lorenz.
\item
  Por último, en \nameref{pcpc} se conjetura el principio de que el caos borra la
  ...
  experimentos realizados en el proyecto, tal y como se aprecia en el capítulo \nameref{practica}. Al mismo tiempo, se cuantifica la pérdida de información de los sistemas caóticos en la sección \nameref{pcpc2} 
\end{itemize}
...
Los datos de Aemet
\refweb{https://deimosestadistica.com/registros-climaticos-espana-basados-la-estadistica/\textbackslash{}\#:\string~:text=AEMET\textbackslash{}\%20ha\textbackslash{}\%20generado\textbackslash{}\%20recientemente\textbackslash{}\%20un,desde\textbackslash{}\%20comienzos\textbackslash{}\%20del\textbackslash{}\%20siglo\textbackslash{}\%20XX}{\ — deimosestadistica.com}{web14} y el conjunto CRU TS 4.05 \refweb{https://crudata.uea.ac.uk/cru/data/hrg/cru_ts_4.05/}{Cru ts 4.05 — crudata.uea.ac.uk}{web15} muestran una tendencia lineal muy suave, con alta variabilidad
interanual.
...
iniciales. Los ficheros con los resultados de las simulaciones se encuentran en el siguiente directorio de Github: \url{https://github.com/ColaCaos/ColaCAOS/tree/master/quarto/caos-libro/05-experimentos/} (aquí veras varios subdirectorios llamados Simulación0, Simulación1, .., Simulación4).
...
un número de decimales muy alto, y en general a la precisión finita de los
...
valores. Esto no sólo lo presenta este sistema, sino que como ya hemos
...
menos tiempo que otras en hacerlo. El motivo por el que cambia de giro es una desalineación del movimiento de las canicas con el sentido de la rotación de la rueda. Hay momentos en el que la rueda parece pararse, y si justamente en uno de esos momentos, las canicas golpean las paredes de la rueda con suficiente fuerza en sentido contrario al giro, entonces se produce el cambio de sentido de rotación. Las canicas son el fenómeno perturbador que hace que la rueda cambie de giro. 
...
angular, posición en X y posición en Y en distintos tiempos.

Otra de las particularidades del atractor es que es muy similar al de Lorenz (ver figura \ref{fig:lorenz}), al menos en apariencia. Es decir, hay dos estados que se alternan de forma caótica. En un instante determinado, resulta imposible de predecir en qué sentido de giro se va a encontrar el sistema. 
...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{Relación inversa entre la predictibilidad y la novedad en un sistema dinámico}
  \label{fig:predictibilidad-novedad}
\end{figure}
...
% Generado a partir de memoria.lof
% ===============================
...
\figline{13.1}{Estructura de los ficheros de observaciones y pronósticos}{45}
\figline{13.2}{Estructura de los ficheros de observaciones y pronósticos}{46}
\figline{13.3}{Error de temperatura en escala lineal}{47}
\figline{13.4}{Error de humedad en escala lineal}{47}
\figline{13.5}{Error de presión en escala lineal}{48}
\figline{13.6}{Error de velocidad del viento en escala lineal}{48}
\figline{13.7}{Error medio absoluto de temperatura en escala lineal}{49}
\figline{13.8}{Error medio absoluto de humedad en escala lineal}{49}
\figline{13.9}{Error medio absoluto de presión en escala lineal}{49}
\figline{13.10}{Error medio absoluto de velocidad del viento en escala lineal}{50}
\figline{13.11}{Error medio absoluto de temperatura y ajuste de regresión}{50}
\figline{13.12}{Error medio absoluto de humedad y ajuste de regresión}{50}
\figline{13.13}{Error medio absoluto de presión y ajuste de regresión}{51}
\figline{13.14}{Error medio absoluto de velocidad del viento y ajuste de regresión}{51}

\figline{16.1}{Órbita de la función logística para $r=3.9$}{57}
\figline{16.2}{Distribución uniforme en función del tiempo}{58}
\figline{16.3}{Diagrama de fase de la función logística para $r=3.9$}{58}
\figline{16.4}{Diagrama de fase de números uniformes entre 0 y 1}{59}
\figline{16.5}{Atractor completo con 2000 iteraciones}{59}
\figline{16.6}{Atractor completo con 10000 iteraciones}{60}
\figline{16.7}{Atractor completo con 15000 iteraciones}{60}
\figline{16.8}{Atractor completo con 20000 iteraciones}{61}
\figline{16.9}{Atractor completo con 50000 iteraciones}{61}
\figline{16.10}{Atractores caóticos para distintos valores de $r$}{62}
\figline{16.11}{Atractores no caóticos para distintos valores de $r$}{62}
\figline{16.12}{Atractor de Lorenz}{62}

\figline{18.1}{Precipitación anual en España (CRU TS 4.05)}{67}

\figline{20.1}{Rueda caótica - Escena completa}{74}
\figline{20.2}{Rueda caótica Zoom}{74}

\figline{21.1}{180 segundos de simulación}{75}
\figline{21.2}{5 segundos de simulación}{76}
\figline{21.3}{Atractor}{77}
\figline{21.4}{Vista desde arriba}{77}
\figline{21.5}{vista lateral del atractor}{78}

\figline{26.1}{Relación inversa entre la predictibilidad y la novedad en un sistema dinámico}{86}
\figline{26.2}{Péndulo simple}{87}
\figline{26.3}{Péndulo doble}{87}
\figline{26.4}{Mapa logístico en zona no caótica}{88}
\figline{26.5}{Mapa logístico en zona caótica}{88}
\figline{26.6}{Rueda caótica sin canicas}{89}
\figline{26.7}{Rueda caótica con canicas}{89}

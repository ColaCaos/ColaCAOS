...
# Se priorizan: 1) presión barométrica, 2) viento (velocidad y dirección), 3) humedad,
# y se añaden las variables omitidas: temperatura y lluvia.
FEATURES = [
    'pressure_msl (hPa)',      # Presión barométrica
    'wind_speed_10m (km/h)',   # Velocidad del viento
    'wind_direction_10m (°)',  # Dirección del viento
    'relative_humidity_2m (%)',# Humedad relativa
    'temperature_2m (°C)',     # Temperatura
    'rain (mm)'                # Cantidad de lluvia
]

# Variable objetivo: probabilidad de lluvia
# Se asume que en el CSV existe la columna 'rain_probability' con valores entre 0 y 1.
...
# Función para normalizar las variables (tanto de entrada como la variable objetivo)
...
class WeatherDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # Ventana de entrada con las variables seleccionadas
        x = self.df.loc[idx : idx + self.input_window - 1, self.input_features].values.astype(np.float32)
        # Ventana de salida: variable objetivo (probabilidad de lluvia)
...
class ForecastNet(nn.Module):
    ...
    def forward(self, src, target_seq=None, teacher_forcing_ratio=0.0):
        ...
        # Se utiliza el último valor de la primera feature (presión barométrica) para iniciar el decoder
        ...
        for t in range(self.output_window):
            ...
            # Activación sigmoidal para que la salida esté en el rango [0, 1]
...
def main():
    ...
    # Crear la columna 'rain_probability' a partir de 'rain (mm)' si no existe
    ...
    # Normalización de todas las variables utilizadas (entradas y salida)
    df_norm, stats = normalize_data(df.copy(), ALL_FEATURES + [OUTPUT_FEATURE])
    
    # Crear el dataset con las variables seleccionadas
    ...
    # Se utiliza MSELoss; se podría evaluar otras funciones de pérdida según la formulación del problema
    ...
    torch.save(model.state_dict(), "model/forecast_modelRain.pt")
    pd.DataFrame(stats).to_csv("model/normalization_statsRain.csv", index=True)
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("model/training_validation_lossRain.png")
    plt.show()

...
OUTPUT_WINDOW = 72    # Predecir 72 horas siguientes
BATCH_SIZE = 32
NUM_EPOCHS = 20
LEARNING_RATE = 1e-3
HIDDEN_SIZE = 64
...
# Modelo de red neuronal: Encoder–Decoder con LSTM
class ForecastNet(nn.Module):
    ...
    def forward(self, src):
        ...
        decoder_input = src[:, -1:, :]  # forma: [batch, 1, num_features]
        outputs = []
        for t in range(self.output_window):
            out, (hidden, cell) = self.decoder(decoder_input, (hidden, cell))
            pred = self.fc(out)  # predicción para el siguiente paso
            outputs.append(pred)
...
def main():
    ...
    # Ordenar cronológicamente si fuera necesario
    df.sort_values('time', inplace=True)
    
    # Normalización de variables (se pueden guardar los parámetros para uso futuro)
    ...
    # Graficar la evolución de la pérdida de entrenamiento y validación por epoch

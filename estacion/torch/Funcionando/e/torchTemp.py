...
class WeatherDataset(Dataset):
    def __init__(self, df, input_window, output_window, input_features, output_features):
    ...
    def __getitem__(self, idx):
        ...
        # Ventana de salida: variables objetivo (temperatura y lluvia)
        y = self.df.loc[idx + self.input_window : idx + self.input_window + self.output_window - 1, self.output_features].values.astype(np.float32)
...
class ForecastNet(nn.Module):
    def __init__(self, num_features, hidden_size, num_layers, output_window, output_dim, dropout=0.0):
        ...
        Args:
            ...
            output_dim: Dimensión de la salida (número de variables objetivo).
        ...
        self.decoder = nn.LSTM(input_size=num_features, hidden_size=hidden_size, 
                               num_layers=num_layers, batch_first=True, 
                               dropout=dropout if num_layers > 1 else 0)
        # Capa lineal que proyecta de hidden_size a output_dim (por ejemplo, 2)
    ...
    def forward(self, src, target_seq=None, teacher_forcing_ratio=0.0):
        ...
        Args:
            ...
            target_seq: Secuencia objetivo para teacher forcing, de forma [batch, OUTPUT_WINDOW, output_dim].
            teacher_forcing_ratio: Proporción de teacher forcing a aplicar.
        Returns:
            outputs: Predicciones de forma [batch, OUTPUT_WINDOW, output_dim].
        """
        batch_size = src.size(0)
        _, (hidden, cell) = self.encoder(src)
        # Inicialización del decoder utilizando los últimos valores conocidos de las variables objetivo.
        # Se asume que en ALL_FEATURES, 'temperature_2m (°C)' se encuentra en la posición 0 y 'rain (mm)' en la posición 2.
        ...
        for t in range(self.output_window):
            ...
            outputs.append(pred)
            # Aplicar teacher forcing si se dispone de secuencia objetivo
            if target_seq is not None and np.random.rand() < teacher_forcing_ratio:
                decoder_input = target_seq[:, t:t+1, :]  # [batch, 1, output_dim]
            else:
                decoder_input = pred
...
def main():
    ...
    # Crear el dataset con todas las features y las variables objetivo (temperatura y lluvia)
    dataset = WeatherDataset(df_norm, INPUT_WINDOW, OUTPUT_WINDOW, ALL_FEATURES, OUTPUT_FEATURES)
    ...
    model = ForecastNet(num_features=len(ALL_FEATURES), hidden_size=HIDDEN_SIZE, 
                        ...
                        output_dim=len(OUTPUT_FEATURES), dropout=DROPOUT)

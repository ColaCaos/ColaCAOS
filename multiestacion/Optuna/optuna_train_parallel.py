...
class ForecastNet(nn.Module):
    ...
    def forward(self, src, target_seq=None, teacher_forcing_ratio=0.0):
        ...
        # Asumimos que temperatura y lluvia de Galapagar están en las posiciones 0 y 2
        decoder_input = src[:, -1:, :][:, :, [0, 2]]
        outputs = []
...
# -------------------------
# Callback para imprimir el progreso de cada trial
# -------------------------
def print_trial_callback(study, trial):
    if trial.state == optuna.trial.TrialState.COMPLETE:
        print(f"Trial {trial.number} completado: Loss = {trial.value:.6f}, Mejor Loss hasta ahora = {study.best_value:.6f}")
...
def objective(trial):
    ...
    # Cargar datos y construir dataset
    ...
    for epoch in range(EPOCHS_OPTUNA):
        ...
        for batch_x, batch_y in train_loader:
            ...
            batch_count += 1
            # Imprimir progreso cada 10 batches
            if batch_count % 10 == 0:
                print(f"Trial {trial.number} - Epoch {epoch+1}/{EPOCHS_OPTUNA} - Batch {batch_count}: Loss = {loss.item():.6f}")
        epoch_loss /= len(train_dataset)
        print(f"Trial {trial.number} - Epoch {epoch+1}/{EPOCHS_OPTUNA} completada. Loss promedio = {epoch_loss:.6f}")
...
# -------------------------
# Main: Crear estudio y optimizar en paralelo con feedback
# -------------------------
def main():
    # Usamos un almacenamiento SQLite para permitir la paralelización
    storage_name = "sqlite:///optuna_study.db"
    study = optuna.create_study(direction="minimize", storage=storage_name, load_if_exists=True)
    
    # Ejecutar optimización en paralelo usando n_jobs > 1 (por ejemplo, 8 trabajadores)
    study.optimize(objective, n_trials=50, n_jobs=8, callbacks=[print_trial_callback])

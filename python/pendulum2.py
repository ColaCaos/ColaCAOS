...
# Compute the center of mass for the original simulation
def calc_center_of_mass(x1, y1, x2, y2, m1, m2):
    x_com = (m1 * x1 + m2 * x2) / (m1 + m2)
    y_com = (m1 * y1 + m2 * y2) / (m1 + m2)
    return x_com, y_com

x_com, y_com = calc_center_of_mass(x1_sim, y1_sim, x2_sim, y2_sim, m1, m2)

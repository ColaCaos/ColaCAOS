...
varianceU = 0.01 * max_u_wind  # 1% of the maximum value
varianceV = 0.01 * max_v_wind  # 1% of the maximum value
...
inner_steps = 4*24  # save model outputs once every 24 hours
outer_steps = 16 * 24 // inner_steps  # total of 4 days
...
# Visualize ERA5 vs NeuralGCM trajectories
#combined_ds.specific_humidity.isel(level=36).plot(
#    x='longitude', y='latitude', row='time', col='model', robust=True, aspect=2, size=2
#);
# Show the plot
#print("show plot")
...
""" 
# Print levels and their indices
print("Available levels and their indices:")
for idx, level in enumerate(combined_ds['level'].values):
    print(f"Index: {idx}, Level: {level}")

print("getting humidity")
humidity_data = combined_ds.specific_humidity.isel(level=36)
longitude = humidity_data['longitude']
latitude = humidity_data['latitude']

print("looping through time and model dimensions")
# Looping through time and model dimensions for custom plotting
for time_index, time_val in enumerate(humidity_data['time']):
    for model_index, model_val in enumerate(humidity_data['model']):
        data_slice = humidity_data.sel(time=time_val, model=model_val)
        plt.figure(figsize=(10, 5))
        plt.contourf(longitude, latitude, data_slice, cmap='viridis')
        plt.colorbar(label='Specific Humidity')
        plt.title(f"Specific Humidity at 850 level\nTime: {time_val.values} | Model: {model_val.values}")

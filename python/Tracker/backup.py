...
def main():
    ...
    while time.time() - t_cal < 5.0:
        ...
        if d1 > 5 and d2 > 5:
            s1 = L1_cm / d1
            s2 = L2_cm / d2
            print(f"Calibrado tras 5s: s1={s1:.3f}, s2={s2:.3f}")
            break

    if s1 is None or s2 is None:
        print("No se pudo calibrar tras 5 segundos.")
        cap.release()
        return

    # Prepara Matplotlib tras calibrar
    plt.ion()
    fig, ax = plt.subplots()
    ...
    while True:
        ...
        # Actualizar plot (solo tras calibrar)

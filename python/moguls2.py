...
def runge_kutta_step(f, x0, dt, *args):
    """
    runge_kutta_step(f, x0, dt, *args)
    ...
    """
    k1 = f(0, x0, *args) * dt
    k2 = f(0, x0 + k1 / 2, *args) * dt
    k3 = f(0, x0 + k2 / 2, *args) * dt
    k4 = f(0, x0 + k3, *args) * dt
...
sol_ex1 = solver(board, x_init, y_init, v_init, u_init, dt=0.01, N_t=n_time, N=n_sleds, b=b)
...
sol_ex2 = solver(board, x_init, y_init, v_init, u_init, dt=0.01, N_t=n_time, N=n_sleds, b=b)
...
sol_ex3 = solver(board, x_init, y_init, v_init, u_init, dt=0.01, N_t=n_time, N=n_sleds, b=b)
...
def bifurcation_diagram(b_range, dt=0.01, N_t=2000, N=10, transient=1000):
    """
    Generate a bifurcation diagram for varying 'b' (height of moguls).

    Parameters:
    - b_range: array of 'b' values to simulate
    - dt: timestep
    - N_t: total number of time steps
    - N: number of initial conditions per 'b'
    - transient: number of initial steps to discard to eliminate transients

    Returns:
    - b_vals: list of 'b' values repeated for each collected 'v'
    - v_vals: list of collected 'v' values after transient
    """
    
    b_vals = []
    v_vals = []
    
    for idx, b_current in enumerate(b_range):
        print(f"Simulating for b = {b_current:.3f} ({idx+1}/{len(b_range)})")
        ...
        # Run the solver directly without using partial
        sol = solver(board, x_init, y_init, v_init, u_init, dt=dt, N_t=N_t, N=N, b=b_current)
        
        # Collect 'v' after transient to focus on steady-state behavior
        v_final = sol[3, transient:, :].flatten()
        
        # Append the results
        b_vals.extend([b_current] * len(v_final))
        v_vals.extend(v_final)
...
print("Starting bifurcation diagram simulation...")
start_bifurcation = time.time()
b_vals, v_vals = bifurcation_diagram(b_values, dt=0.01, N_t=2000, N=10, transient=1000)
...
plt.scatter(b_vals, v_vals, s=0.5, color='blue', alpha=0.5)
plt.xlabel('b - Height of Moguls', fontsize=13)
plt.ylabel('v - Cross Slope Velocity (m/s)', fontsize=13)
plt.title('Bifurcation Diagram: Cross Slope Velocity vs Height of Moguls', fontsize=15)
plt.grid(True)
plt.tight_layout()
plt.savefig('bifurcation_diagram.png')  # Save the bifurcation diagram
plt.show()

print("Bifurcation diagram plotted successfully.")

...
def initPendulums(theta1, theta2, v1, v2):
    ...
    The initial angles for both pendulums are mapped from 150° to 151°
    (converted to radians). This creates a very fine sampling over that small range.
    """
    i = cuda.blockIdx.x * cuda.blockDim.x + cuda.threadIdx.x
    j = cuda.blockIdx.y * cuda.blockDim.y + cuda.threadIdx.y
    if i < WIDTH and j < HEIGHT:
        idx = j * WIDTH + i
        # Map horizontal coordinate i to theta1 in [150, 151] degrees, then to radians.
        theta1[idx] = (150.0 + i * (1.0 / (WIDTH - 1))) * math.pi / 180.0
        # Map vertical coordinate j to theta2 in [150, 151] degrees, then to radians.
        theta2[idx] = (150.0 + j * (1.0 / (HEIGHT - 1))) * math.pi / 180.0
        v1[idx] = 0.0
        v2[idx] = 0.0
...
def updatePendulums(theta1, theta2, v1, v2, dt):
    ...
    Uses the standard double pendulum equations with m1 = m2 = 1, l1 = l2 = 1,
    and g = 9.81.
    """
    i = cuda.blockIdx.x * cuda.blockDim.x + cuda.threadIdx.x
    j = cuda.blockIdx.y * cuda.blockDim.y + cuda.threadIdx.y
...
def updateColors(theta1, theta2, img):
    ...
    Here the value is derived from the average of the normalized sine values of θ₁ and θ₂.
    Normalization maps sin(θ) from [-1, 1] to [0, 1], and then we scale to 0–255.
    """
    i = cuda.blockIdx.x * cuda.blockDim.x + cuda.threadIdx.x
    j = cuda.blockIdx.y * cuda.blockDim.y + cuda.threadIdx.y
    if i < WIDTH and j < HEIGHT:
...
def main():
    ...
    # Allocate device arrays for the simulation state.
    ...
    # Initialize the pendulum states.
    ...
    while True:
        ...
        # Update the grayscale image based on the updated angles.
        updateColors[blocks_per_grid, threads_per_block](theta1, theta2, img)
        cuda.synchronize()

        # Copy the image from device to host and reshape it.
        host_img = img.copy_to_host().reshape((HEIGHT, WIDTH))
        # Apply a colormap (here, JET) to map the grayscale to a 256-color image.
        colored_img = cv2.applyColorMap(host_img, cv2.COLORMAP_JET)

        # Display the resulting image.
        cv2.imshow("Double Pendulum", colored_img)
        if cv2.waitKey(1) == 27:  # Exit on 'Esc' key press.
            break
